...
| **2** | **AdGuard Home (LXC)** | `15s` | `15s` | **Resolução de Nomes (DNS).** Leve e rápido (Alpine). Deve subir imediatamente após a rede para garantir que o `auth.home` seja resolvível para o resto da infra. |
| **3** | **Management (LXC)** | `30s` | `30s` | **Torre de Controle.** Necessário para rodar playbooks de correção ou automação logo após o boot. |
| **4** | **DockerHost (VM)** | `60s` | `180s` | **Camada de Aplicação.** Onde rodam Traefik, Authentik e Apps. Depende de Rede e DNS para estarem estáveis. Shutdown longo configurado para permitir `docker stop` gracioso dos containers (evita corrupção de DB). |
| **5** | **OrangeShadow (VM)** | `0s` | `600s` | **Carga Pesada.** Sobe por último. Não tem dependentes. Shutdown estendido (5 min) pois o `bitcoind`, por exemplo, demora para descarregar o cache de memória para o disco (flush) ao desligar. |
