...
| Firewall Nativo | **Exceção Gerenciada:** O DockerHost utiliza regras de `iptables` manuais persistidas via Systemd (`tailscale-nat.service`) para permitir roteamento VPN e NAT (Masquerade), contornando intencionalmente o isolamento padrão do Docker para tráfego da interface `tailscale0`. | Necessário para funcionamento do Subnet Router. |
