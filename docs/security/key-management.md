...
|---|---|---|
| `configuration/inventory/group_vars/dockerhost/secrets.sops.yaml` | Grupo `dockerhost` | `tailscale_authkey`, `authentik_secret_key`, `authentik_db_password`, `grafana_password`, `healthchecks_uuid`, `pve_password`, `ntfy_token`, `vaultwarden_admin_token`, `speedtest_app_key`, `freshrss_db_password`, `freshrss_admin_password`, `media_vpn_wireguard_private_key`, `filebrowserquantum_admin_password` |
