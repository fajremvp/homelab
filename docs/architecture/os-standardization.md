...
| :--- | :--- | :--- | :--- |
| **Infraestrutura Crítica (VMs)** | **Debian Stable (Minimal)** | O "Padrão Ouro". Usa `glibc` (máxima compatibilidade com containers proprietários) e `systemd`. Estabilidade absoluta para serviços que não podem falhar. | DockerHost, Vault, OrangeShadow. |
